...
function index() {
  return ProfileModel.find().lean().exec().then((profiles) => profiles);
}
function get(id) {
  return ProfileModel.findOne({ id }).lean().exec().then((profile) => profile);
}
function create(profile) {
  const p = new ProfileModel(profile);
  return p.save().then((savedProfile) => savedProfile.toObject());
}
